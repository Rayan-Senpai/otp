...
echo "\n";
echo "\n";
echo color("yellow","[?] Masukan Token :");
$input_hp = fopen("php://stdin","r");
$idhp = trim(fgets($input_hp));
$idhp = "\"$idhp\"";
echo color("blue","| Script KLIK INDOMARET & IDM POINT \n");
echo color("red","| ------------IPUR27------------ \n");
echo color("yellow","| MY NAME : IPUR27 \n");
echo color("yellow","| @IPUR27 \n");
...
if ($inputan == "1"){ //Login
    ...
    echo color("yellow", "| Masukan Email/No Handphone : ");
    $input_hp = fopen("php://stdin","r");
...
#---------------------------------------------------------------------------------------------------------
    ...
    if(file_exists("config.json")){
        ...
        $ar2 = $arr2['name'];
        $ar1 = $arr2['password'];
        }else{ echo color("red","| Anda harus menyeting Nama dan Password default!!"); goto welcome;}
...
echo "   / _ .. _ \ ".PHP_EOL;
echo "  //  |  |  \\ Indomaret Pointku (SPECIAL OTP)".PHP_EOL;
echo " //   |  |   \\ BANG IPUR27 (SCRIPT PRIBADI)".PHP_EOL;
echo " ||  / /\ \  || ".PHP_EOL;
...
fwrite($file,"--------------------------------------------------------".PHP_EOL);  
fclose($file);  
echo 'KUPON SUKSES DIAMBIL!!!'.PHP_EOL;
